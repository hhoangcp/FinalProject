...
def extract_plot_data(dataset_key, metric_key):
    dataset = sorted(data[dataset_key], key=lambda x: x["Beta"])  # Sort by Beta
    alpha = [entry["Beta"] for entry in dataset]
...
plt.tight_layout()
plt.savefig("./Beta_CIRR_Dataset.png")
...
plt.legend(loc='upper right', bbox_to_anchor=(1, 1))
plt.tight_layout()
plt.savefig("./Beta_FashionIQ_Dataset.png")
...
plt.legend(loc='upper right', bbox_to_anchor=(1, 1))
plt.tight_layout()
plt.savefig("./Beta_CIRCO_Dataset.png")
